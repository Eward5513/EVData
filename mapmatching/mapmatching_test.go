...
var _ = Describe("mapmatching test", func() {
	...
	Context("MergePoints test", func() {
		It("should get right result #1", MustPassRepeatedly(1), func() {
			...
			wg.Wait()
			Expect(t.Vin).To(Equal(int32(1)))
			Expect(t.Tid).To(Equal(int32(0)))

			for i := range t.Rps {
				Expect(t.Rps[i]).To(BeIdenticalTo(msg[i]))
			}

			res := mapmatching.MergePoints(t.Mps, t.Rps)
